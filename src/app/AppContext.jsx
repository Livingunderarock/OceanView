...
import {
  getClientOceanModelSlice,
  getClientOceanCurrentSlice,
  getClientArgoProfiles,
  getClientGliderMissions,
  getClientCTDStations,
  getClientBGCProfiles,
} from '../services/oceanClientDataService.js';
...
async function safeFetchJson(endpoint, fallbackFn, timeoutMs = 2500) {
  ...
  try {
    ...
    // Network error, 404, or abort -> fallback to verified client fixture
  }
  return await fallbackFn();
}
import {
...
export function AppProvider({ children }) {
  ...
  const fetchModelGrid = useCallback(async (selection) => {
    ...
    try {
      ...
      const modelJson = await safeFetchJson(url, () =>
        getClientOceanModelSlice({
          variable: selection.variable,
          depth: selection.depthMeters,
          time: selection.timeValue,
          minLat,
          maxLat,
          minLon,
          maxLon,
  ...
  const fetchCurrentVectors = useCallback(async (selection) => {
    ...
    try {
      const currentUrl = `/api/ocean/current/slice?minLat=${minLat}&maxLat=${maxLat}&minLon=${minLon}&maxLon=${maxLon}&stride=2&depth=${selection.depthMeters}`;
      const currentJson = await safeFetchJson(currentUrl, () =>
        getClientOceanCurrentSlice({
          depth: selection.depthMeters,
          minLat,
          maxLat,
  ...
  const fetchArgo = useCallback(async (selection) => {
    ...
    try {
      const argoUrl = `/api/ocean/argo/profiles?minLat=${minLat}&maxLat=${maxLat}&minLon=${minLon}&maxLon=${maxLon}&maxProfiles=10`;
      const argoJson = await safeFetchJson(argoUrl, () =>
        getClientArgoProfiles({ minLat, maxLat, minLon, maxLon, maxProfiles: 10 })
      );
      if (argoJson) {
  ...
  const fetchGlider = useCallback(async () => {
    ...
    try {
      const gliderJson = await safeFetchJson('/api/ocean/glider/missions', getClientGliderMissions);
      if (gliderJson) {
  ...
  const fetchCTD = useCallback(async () => {
    ...
    try {
      const ctdJson = await safeFetchJson('/api/ocean/ctd/stations', getClientCTDStations);
      if (ctdJson) {
  ...
  const fetchBGC = useCallback(async () => {
    ...
    try {
      const bgcJson = await safeFetchJson('/api/ocean/bgc/profiles', getClientBGCProfiles);
      if (bgcJson) {
